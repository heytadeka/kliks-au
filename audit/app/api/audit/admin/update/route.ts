...
export async function PATCH(req: NextRequest) {
  ...
  const {
    prospect_id,
    brand_name, store_url, prospect_name, prospect_email, niche, cta_link, gmb_cid, aov,
  ...
  const aovValue = aov === '' || aov == null ? null : Number(aov)

  const { error: prospectError } = await supabaseAdmin
    .from('prospects')
    .update({ brand_name, store_url, prospect_name, prospect_email, niche, cta_link, gmb_cid: gmb_cid || null, aov: aovValue })
    .eq('id', prospect_id)
