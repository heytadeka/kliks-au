...
export async function PATCH(req: NextRequest) {
  ...
  const {
    prospect_id,
    brand_name, store_url, prospect_name, prospect_email, niche, cta_link, gmb_cid,
  ...
  const { error: prospectError } = await supabaseAdmin
    .from('prospects')
    .update({ brand_name, store_url, prospect_name, prospect_email, niche, cta_link, gmb_cid: gmb_cid || null })
    .eq('id', prospect_id)
