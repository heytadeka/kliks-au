...
export async function POST(req: NextRequest) {
  ...
  const { error: insertError } = await supabaseAdmin.from('outreach_log').insert({
  ...
  return NextResponse.json({ success: true, slug: prospect.slug, brand_name: prospect.brand_name })
}
