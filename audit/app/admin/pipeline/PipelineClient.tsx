...
export default function PipelineClient({ initialDomains }: { initialDomains: any[] }) {
  ...
  const [lastResult, setLastResult] = useState<{ shopify: number; squarespace: number } | null>(null)
  ...
  async function handleDiscover() {
    ...
    try {
      ...
      setLastResult({ shopify: data.shopify_found ?? 0, squarespace: data.squarespace_found ?? 0 })
      // Refresh domain list from server
      const refreshRes = await fetch('/api/pipeline/domains')
      if (refreshRes.ok) {
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: S.muted, marginBottom: 20 }}>
            {domains.length} domains monitored — {shopifyCount} Shopify, {squarespaceCount} Squarespace
          </p>
                  ...
                  {domains.map((row: any, i: number) => (
                    <tr key={row.id} style={{ background: i % 2 === 0 ? S.bg2 : S.bg }}>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        {row.traffic_drop_pct != null ? (
                          <span style={{ color: row.traffic_drop_pct > 15 ? '#ef4444' : S.muted, fontWeight: row.traffic_drop_pct > 15 ? 600 : 400 }}>
                            ↓ {Math.round(row.traffic_drop_pct)}%
                          </span>
                        ) : '-'}
                      </td>
                      <td style={{ padding: '12px 14px' }}>
                        {row.flagged ? <span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', background: '#ef4444' }} /> : '-'}
                      </td>
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          {row.status !== 'ignored' && (
