...
export default async function TodayPage() {
  ...
  // open_count > 0 overrides status here - a real-data pull showed several
  // audit_created rows (email_sent_at never set) with real opens already
  // recorded, because the "opened" auto-transition only fires from status
  // email_sent (see gate/route.ts) and Mark Sent doesn't always get clicked
  // even when the link genuinely went out and was viewed. Without this override
  ...
  const readyToReachOut = rows
    .filter(r => !r.outreach || (r.outreach.status === 'audit_created' && !(r.outreach.open_count > 0)))
